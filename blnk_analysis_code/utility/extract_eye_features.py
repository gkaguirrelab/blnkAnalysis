...
# Construct the path to the pylids directory and import it
blnk_analysis_dir: str = str(pathlib.Path(__file__).parents[2])
pylids_dir: str = os.path.join(blnk_analysis_dir, "pylids")
assert os.path.exists(pylids_dir) and len(os.listdir(pylids_dir)) > 0, f"Pylids dir either does not exist or is empty"
import pylids
...
def generate_playable_video(video: str | np.ndarray, eye_features: list[dict],
    ...
    actual_video_framecount: int = video_io.inspect_video_frame_count(video) if isinstance(video, str) else len(video)
    ...
    video_fps: float = video_io.inspect_video_FPS(video)
    write_process: object = mp.Process(target=video_io.frames_to_video, 
...
def pupil_labs_analyze_video(video: str | np.ndarray, 
                             is_grayscale: bool=False,
                             pupil_focal_length: float=561.5, 
                             pupil_frame_shape: list[int] = [400, 400],
                             pupil_fps: float=120
                             ) -> list[dict]:
    ...
    camera: object = CameraModel(focal_length=pupil_focal_length, resolution=pupil_frame_shape)
    ...
    if(isinstance(video, np.ndarray)):
        ...
        for frame_num, frame in enumerate(video):
            ...
            result_2d["timestamp"] = frame_num / pupil_fps
...
def extract_pupil_features(video: str | np.ndarray,
    ...
    if(method == "pylids"):
        ...
        perimeter_info_dict["size"] = video_io.inspect_video_framesize(video) if isinstance(video, str) else video.shape[1:3]
